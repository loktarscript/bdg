/*/********************************
    Controller: Dic_situacion_amorosa 
...
const update = (req) => {
    let bdg = {
        sid_dic_sit_amorosa: req.params.id,
        nombre: req.body.nombre,
        activo: req.body.activo
    }
    console.log(bdg)
    return new Promise((resolve, reject) => {
        dbConn.query(store.showOne, [req.params.id], (error, bonding) => {
            ...
            if (bonding[0].length > 0) {
                dbConn.query(store.updateData, [bdg.sid_dic_sit_amorosa, bdg.nombre, bdg.activo], (error, dicSexos) => {
                    if (error) reject(error);
...
const deleteObj = (req) => {
    let sid_dic_sit_amorosa = req.params.id;
    return new Promise((resolve, reject) => {
        dbConn.query(store.showOne, [req.params.id], (error, bonding) => {
            if (error) reject(error);
            if (bonding[0].length > 0) {
                dbConn.query(store.deleteData, [sid_dic_sit_amorosa], (error, registro) => {
                    if (error) reject(error);
